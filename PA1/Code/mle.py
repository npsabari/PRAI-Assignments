...
def multinomial():
    ...
    threshold = [0.001, 0.01, 0.02, 0.05, 0.08, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
    for thres in threshold:
        print 'For threshold '+str(thres)
        for strt in range(0, 10, 2):
            ...
            while cnt < 2:
                ...
                for filename in files:
                    tokens = list()
                    log_prob_ns = math.log(docs_ns_size*1.0/(docs_ns_size+docs_s_size))
                    log_prob_s = math.log(docs_s_size*1.0/(docs_ns_size+docs_s_size))
                    ...
                    for token in tokens:
                        ...
                        log_prob_ns += math.log((t+1.0)/(local_ns_size+vocab_size))
                        t = 0
                        if token in local_s_word_count: t = local_s_word_count[token]
                        log_prob_s += math.log((t+1.0)/(local_s_size+vocab_size))
                    #print 'log_ns %f, log_s %f' % (log_prob_ns, log_prob_s)
                    # Classified as spam
                    #if (log_prob_s/(log_prob_ns+log_prob_s)) >= thres:
                    if log_prob_s - log_prob_ns > abs(log_prob_s*thres):
